...
Or per-file with options overridden inline:
...
Generate it with the companion L<gen-stopwords> script:
...
sub set_stopwords_file {
    $_config{stopwords_file} = $_[0]
}
...
sub get_stopwords_file {
    $_config{stopwords_file}
}
...
sub all_pod_files_spelling_ok {
    ...
    my $stopwords_file = $_config{stopwords_file};

    unless (-e $stopwords_file) {
        $TB->plan(
            skip_all =>
            "No $stopwords_file found. Run gen-stopwords to create one."
        );
        return;
    }
...
# Load stopwords from the configured file.
# Returns a hashref of lowercased words, or undef on failure.
sub _load_stopwords {
    ...
    open my $fh, '<', $file
        or do { $TB->diag("Cannot open $file: $!"); return {} };
...
# Spell-check a single file. Returns ( $passed, \%errors ).
# %errors is keyed on word, value is arrayref of line numbers.
sub _check_file {
    my ($file, $stopwords) = @_;
    ...
    open my $fh, '<', $file
        or do { return (0, { _open_error => ["Cannot open: $!"] }) };
...
L<Pod::Wordlist> is not a runtime dependency of this module — it is a
dependency of the companion L<gen-stopwords> script.
